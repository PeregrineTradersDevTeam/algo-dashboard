...
export default {
  ...
  methods: {
    ...
    getCurveData: function() {
      let that = this;
      let minFrom = that.series[0]?.data.length;
      if (that.series.length > 0) {
        if (minFrom < that.series[1]?.data.length) {
          minFrom = that.series[1].data.length;
        }
      }

      fetch(`/api/pnl/${that.code}/multiarray?from=${minFrom}`)
        .then((r) => r.json())
        .then((res) => {
          if (res == null) {
            return;
          ...
          that.series[0].data.push(...res.negative.data);
          that.series[1].data.push(...res.positive.data);

          if (res.negative.data.length > 0 || res.positive.data.length > 0) {
            ...
            //   colorIndex = 0;
            // }
            // that.chartOptions.fill.colors = [that.colors[[0][0]]];
            // that.chartOptions.stroke.colors = [that.colors[0][1]];
            // that.chartOptions;
